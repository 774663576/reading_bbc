...
class ArticleInfo:
    ...
    update_time: str
    views: int
    catetory:take-away-english

class BBCLearningEnglishScraper:
    def __init__(self, base_output_dir: str = 'take-away-english'):
        """初始化爬虫配置"""
...
def main():
    ...
    if article_info:
        ...
        print(f"封面: {article_info.cover}")
        print(f"MP3 URL: {article_info.mp3_url}")
        print(f"PDF URL: {article_info.pdf_url}")
        print(f"更新时间: {article_info.update_time}")
        print(f"浏览量: {article_info.views}")
    else:
        print("爬取文章失败")
        exit(1)
